...
import { Candle, CandleApiResponse, chartData } from '../interfaces/candle.interface';
...
export class SpecificCryptoPriceService {
  ...
  fetchCandles(cryptoName: string): Observable<chartData> {
    return this.http.get<CandleApiResponse>(`https://api.coincap.io/v2/assets/${cryptoName}/history?interval=d1`)
      .pipe(
        map(response => {
  ...
  formatCandles(candles: Candle[]): chartData {
    const labels = candles.map(item => new Date(item.time).toLocaleDateString());  
